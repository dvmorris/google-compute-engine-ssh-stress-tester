...
# run the tests
# ssh -o "StrictHostKeyChecking no" -vvv -i ~/.ssh/id_rsa_test ${SERVER_NAME} uptime
{ time parallel -kj${PARALLEL_NUM_WORKERS} --tag "ssh -o \"StrictHostKeyChecking no\" -i ~/.ssh/id_rsa_test ${SERVER_NAME} uptime" ::: $(seq 00 $PARALLEL_NUM_ITERATIONS) > $TEST_NAME.log 2>&1 ; } 2> time.txt

# gather test results
OUTPUT_DATA="${TEST_NAME},${ENABLE_OSLOGIN},${MACHINE_TYPE},${ZONE},${VM_IMAGE},${SSHD_MAX_STARTUPS},${PARALLEL_NUM_WORKERS},${PARALLEL_NUM_ITERATIONS}"
